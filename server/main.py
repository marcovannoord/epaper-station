...
from .models import TagInfo, User, Device
from . import db
...
def create_post():
    ...
    ).first()  # if this returns a result, then the platform already exists in database
    if platform:
        ...
        if request.form["device"] and request.form["device"] != "--" and request.form["macaddr"]:
            ...
            if len(__mac) == 12:
                ...
                if not known_device and known_platform:  # device was not known before, but platform is valid
                    ...
                    db.session.commit()
                    flash(
                        "Success: Added previously unkown device {} to whitelist of {}".format(
                            __mac, known_platform.name
                        ),
                        "warning",
                    )
                    return render_template("whitelist.html", platforms=platforms, unbound_devices=unbound_devices)
                if known_device and known_platform:  # if the device and platform are known, whitelist the device
                    ...
                    if (
                        request.form.get("notes") and request.form.get("notes") != ""
                    ):  # Make sure we do not overwrite existing notes
...
def update():
    ...
    if __dev and __mac and __ver and len(__mac) == 12:
        ...
        if platform:  # device is known for a platform
            device_whitelisted = (
                Platform.query.join(Device)
                .filter(Device.mac == __mac)
                .filter(Platform.name == __dev)
                .first()  # check if the device is whitelisted and is requesting the correct firmware
            )
            if device_whitelisted:
                ...
                if __ver < version.parse(platform.version):
                    ...
                    if os.path.isfile(path):
                        ...
                        log_event(
                            "INFO: Updating {} of type {} from {} to {} ".format(
                                __mac, platform.name, __ver, platform.version
                            )
                        )
                        return response
                    else:
        ...
        log_event(
            "ERROR: Invalid parameters. __dev: {} and __mac: {} and __ver:{} and len(__mac): {} ".format(
                __dev, __mac, __ver, len(__mac)
            )
        )
        return "Error: Invalid parameters.", 400
...
def upload_post():
    ...
    if file.filename == "" or not allowed_ext(file.filename):
        flash(
            "Error: File upload error or wrong extension. Make sure you upload a file with the extension(s): {}".format(
                str(current_app.config["ALLOWED_EXTENSIONS"])
            )
        )
        return redirect(request.url)
    if file and allowed_ext(file.filename):
        ...
        for platform in platforms:
            m = re.search(
                b"update\?dev=" + platform.name.encode("UTF-8") + b"&ver=(v\d+\.\d+\.\d+)\x00", data, re.IGNORECASE
            )
            if m:  # platform found!
                __ver = m.groups()[0][1:].decode("utf-8")
                # check if the uploaded file is an update to the version that we have in the database
                if (platform.version is None) or (
                    platform.version and version.parse(platform.version) < version.parse(__ver)
                ):
                    ...
                    flash(
                        "Success: File uploaded for platform {} with version {}.".format(platform.name, __ver),
                        "warning",
                    )
                    return redirect(url_for("main.whitelist"))
                else:
